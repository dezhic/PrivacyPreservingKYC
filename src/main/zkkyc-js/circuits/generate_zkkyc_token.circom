...
template GenerateZkKYCToken(n248Bits) {
    ...
    signal output encryptedPayload[4*n248Bits*256+128];  // 4 n*248-bit DIDs + 128-bit AES-CTR IV
    ...
    /* Step 2: Create an encrypted payload that contains (DID_I, DID_HI, DID_HV, DID_V) */
    ...
    encryptedPayload <== messageEncrypt.ciphertext;
 ...
 * - govPubKey: showing that the government is able to decrypt the token
   ...
   - issuerPubKey: to convince verifier that the encrypted payload contains the correct DID_I, DID_HI, which will be used to find the Issuer to retrieve the real identity of the Holding
