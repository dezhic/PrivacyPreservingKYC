...
Privacy-Preserving KYC \# Abstract Know-Your-Customer (KYC) process is a
...
\hypertarget{problems-and-objective}{%
\section{Problems and Objective}\label{problems-and-objective}}
...
\begin{quote}
\_\_Copy Problem:\_\_ When users have to share personal
...
\_\_Bundling Problem:\_\_ While AML/CTF regulations usually
...
\_\_Recursive Oversight Problem:\_\_ When users have to share
...
\hypertarget{the-zkkyc-solution-concept}{%
\section{The zkKYC Solution Concept}\label{the-zkkyc-solution-concept}}
...
\hypertarget{self-sovereign-identity-ssi-using-hyperledger-aries}{%
\section{Self-Sovereign Identity (SSI) Using Hyperledger
Aries}\label{self-sovereign-identity-ssi-using-hyperledger-aries}}

\hypertarget{what-is-self-sovereign-identity}{%
\subsection{What is Self-Sovereign
Identity?}\label{what-is-self-sovereign-identity}}

\hypertarget{key-components-in-ssi}{%
\subsubsection{Key Components in SSI}\label{key-components-in-ssi}}

\textbf{Decentralized Identifier (DID)}

\textbf{Verifier Credential (VC)}

\textbf{Verifiable Data Registry (VDR)}

\hypertarget{hyperledger-ssi}{%
\subsection{Hyperledger SSI}\label{hyperledger-ssi}}

\hypertarget{the-ecosystem}{%
\subsubsection{The Ecosystem}\label{the-ecosystem}}

\includegraphics{eco.png} \textbf{Hyperledger SSI Ecosystem - Aries,
Indy and Ursa}

\hypertarget{hyperledger-aries}{%
\subsubsection{Hyperledger Aries}\label{hyperledger-aries}}

\includegraphics{aca-py.png} \textbf{Hyperledger Aries Architecture}

\hypertarget{implementing-zkkyc-with-hyperledger-aries}{%
\subsection{Implementing zkKYC with Hyperledger
Aries}\label{implementing-zkkyc-with-hyperledger-aries}}

{[}TODO: procedure to issue / verify credentials{]}

\hypertarget{zero-knowledge-proof-zkp-with-zk-snarks}{%
\section{Zero Knowledge Proof (ZKP) with
zk-SNARKs}\label{zero-knowledge-proof-zkp-with-zk-snarks}}
...
\hypertarget{zk-snark-fundamentals}{%
\subsection{zk-SNARK Fundamentals}\label{zk-snark-fundamentals}}

In this section, we will introduce the fundamentals of zk-SNARKs, and
how we can implement zk-SNARKs in our project.

\hypertarget{what-are-zk-snarks}{%
\subsubsection{What are zk-SNARKs?}\label{what-are-zk-snarks}}
...
\hypertarget{choosing-a-proving-system}{%
\subsubsection{Choosing a Proving
System}\label{choosing-a-proving-system}}
...
\hypertarget{implementing-zk-snarks-in-the-project}{%
\subsubsection{Implementing zk-SNARKs in the
Project}\label{implementing-zk-snarks-in-the-project}}
...
\hypertarget{zkp-workflow-for-zkkyc}{%
\subsection{ZKP Workflow for zkKYC}\label{zkp-workflow-for-zkkyc}}
...
\hypertarget{signing-and-verification-with-eddsa}{%
\subsubsection{Signing and Verification with
EdDSA}\label{signing-and-verification-with-eddsa}}
...
\hypertarget{message-representation-in-elgamal-encryption}{%
\subsubsection{Message Representation in ElGamal
Encryption}\label{message-representation-in-elgamal-encryption}}
...
\hypertarget{token-encryption-and-decryption-with-elgamal-and-aes}{%
\subsubsection{Token Encryption and Decryption with ElGamal and
AES}\label{token-encryption-and-decryption-with-elgamal-and-aes}}
...
\begin{Highlighting}[]
\NormalTok{[}
\NormalTok{    c1X}\OperatorTok{,}\NormalTok{ c1Y}\OperatorTok{,}  \CommentTok{// circuit output}
\NormalTok{    c2X}\OperatorTok{,}\NormalTok{ c2Y}\OperatorTok{,}  \CommentTok{// circuit output}
    \OperatorTok{...}\NormalTok{encryptedPayload}\OperatorTok{,} \CommentTok{// circuit output}
\NormalTok{    aesKeyXmXor  }\CommentTok{// circuit public input}
\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{bit-lengths-of-components-248-253-254-or-256}{%
\subsubsection{Bit Lengths of Components --- 248, 253, 254 or
256?}\label{bit-lengths-of-components-248-253-254-or-256}}
...
\hypertarget{interfaces-and-packaging}{%
\subsubsection{Interfaces and
Packaging}\label{interfaces-and-packaging}}
...
\endlastfoot
\textbf{Issuer} \\
\texttt{generateKeyPair(privKey?:\ string)\ =\textgreater{}\ \{\ priv:\ string,\ pub:\ {[}string,\ string{]}\ \}}
...
\texttt{signDidRecord(didI:\ string,\ didHI:\ string,\ privKey:\ string)\ =\textgreater{}\ \{\ s:\ string,\ r:\ string{[}{]}\ \}}
~ Sign the DID record with the private key. \\
\textbf{Holder} \\
\texttt{generateZkKycProof(didI:\ string,\ didHI:\ string,\ didHV:\ string,\ didV:\ string,\ sigS:\ string,\ sigR:\ string{[}{]},\ issuerPubKey:\ string{[}{]},\ govPubKey:\ string{[}{]})\ =\textgreater{}\ \{\ proofJson:\ string,\ publicJson:\ string\ \}}
~ Generate the zkKYC proof and public information, including the
encrypted contents and public keys. \\
\textbf{Verifier} \\
\texttt{parsePublic(publicJson:\ string)\ =\textgreater{}\ \{\ aesKeyPointCipher:\ \{\ c1:\ string{[}{]},\ c2:\ string{[}{]}\ \},\ encryptedPayload:\ string,\ didHV:\ string,\ didV:\ string,\ issuerPubKey:\ string{[}{]},\ govPubKey:\ string{[}{]},\ aesKeyXmXor:\ string\ \}}
...
\texttt{verifyZkKycProof(proofJson:\ string,\ publicJson:\ string)\ =\textgreater{}\ boolean}
~ Verify the zkKYC proof and public information to check if the
information is valid. \\
\textbf{Government} \\
\texttt{decryptToken(parsedPublic:\ string,\ privKey:\ string)\ =\textgreater{}\ \{\ didI:\ string,\ didHI:\ string,\ didHV:\ string,\ didV:\ string\ \}}
...
\hypertarget{requirements-review}{%
\section{Requirements Review}\label{requirements-review}}
...
\hypertarget{project-code-navigation}{%
\subsection{Project Code Navigation}\label{project-code-navigation}}

\hypertarget{circom-snarkjs}{%
\subsubsection{Circom \& SnarkJS}\label{circom-snarkjs}}

\hypertarget{circuits}{%
\paragraph{Circuits}\label{circuits}}
...
\hypertarget{hyperledger-aries-1}{%
\subsubsection{Hyperledger Aries}\label{hyperledger-aries-1}}
